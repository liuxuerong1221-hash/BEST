<template>
  <BasePanel class="turnstile-list" title="摆闸设备列表">
    <!-- 搜索 + 筛选 -->
    <div class="turnstile-list__toolbar">
    ...
    <!-- 表格 -->
    <div class="turnstile-list__table">
      <div class="turnstile-row turnstile-row--header">
        <span class="turnstile-row__name">设备名称</span>
        <span class="turnstile-row__location">所在位置</span>
        <span class="turnstile-row__status">状态</span>
      </div>

      <div class="turnstile-list__body">
        <div
          v-for="g in filtered"
          :key="g.id"
          class="turnstile-row turnstile-row--selectable"
          :class="{ 'turnstile-row--selected': g.id === selectedId }"
          role="button"
          tabindex="0"
        ...
        >
          <span class="turnstile-row__name">{{ g.name }}</span>
          <span class="turnstile-row__location">{{ g.location }}</span>
          <span class="turnstile-row__status">
    ...
    <!-- 分页器 -->
    <div class="turnstile-list__pagination">
      <button
...
function statusLabel(s: TurnstileStatus) { return STATUS_TEXT[s] }
...
const filter = ref<'all' | TurnstileStatus>('all')
const filterOptions = [
...
function selectFilter(v: 'all' | TurnstileStatus) {
  filter.value = v
